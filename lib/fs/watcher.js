...
class Watcher {
  ...
  _create_folder() {
    ...
    try {
      ...
      throw new Error(
        `Failed to create folder "${this.folder_name}": ${e.message}`
      );
    }
  }
  ...
  _append_to_sync_ignore_list() {
    const wf_ignore = chokidar.watch(this.final_path, {
      ignored: (filePath, stats) =>
        stats?.isFile() && !filePath.endsWith('.syncignore'),
    });
    wf_ignore
  ...
  start() {
    if (!this._check_path_exists()) {
      // check path and folder validity
      throw new Error(
        `path is invalid. Enter a valid path. given: ${this.folder_name}`
      );
    }
    try {
